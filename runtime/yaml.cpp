...
void mixed_to_yaml_node(const mixed &data, YAML::Node &node) {
  if (!data.is_array()) {
    ...
    node.push_back(data.as_string().c_str());
    return;
  } // TODO: for maps also
  for (auto it = data.as_array().begin(); it != data.as_array().end(); ++it) {
...
//  if (pos != 0)
//    php_warning("Value of pos other than 0 is not supported. Assigning 0 instead of %d", pos);
  YAML::Node node;
  if (pos > 0) {
    ...
    php_warning("Argument \"pos\" cannot be negative, but it is %d. Setting to default (0)", pos);
    node = YAML::Load(data.c_str());
  }
  else node = YAML::Load(data.c_str());
//  std::string std_data(data.c_str());
//  node = YAML::Load(data.c_str());
//  node >> std_data;
  mixed parsed_data;
